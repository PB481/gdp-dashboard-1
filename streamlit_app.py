...
if uploaded_file is not None:
    ...
    if not df.empty:
            ...
            <body>
                ...
                <div style="display: flex; justify-content: space-around; flex-wrap: wrap;">
                    <div class="chart-container" style="flex: 1; min-width: 30%;">
                        {portfolio_alloc_chart_html}
                    </div>
                    <div class="chart-container" style="flex: 1; min-width: 30%;">
                        {sub_portfolio_alloc_chart_html}
                    </div>
                    <div class="chart-container" style="flex: 1; min-width: 30%;">
                        {brs_alloc_chart_html}
                    </div>
                </div>
            """
            if selected_project_name != 'Select a Project' and project_details is not None and fig_project_monthly is not None:
                monthly_breakdown_df_html = monthly_breakdown_df.style.format({
                    'Actuals': "${:,.2f}",
                    'Forecasts': "${:,.2f}",
                    'Capital Plan': "${:,.2f}"
                }).to_html()
        ...
        if st.button("Generate Report (HTML)"):
            ...
            project_details_table_html = project_details_table.to_html()

            monthly_trends_chart_html = ""
            if monthly_combined_df is not None and not monthly_combined_df.empty:
                monthly_trends_chart_html = fig_monthly_trends.to_html(full_html=False, include_plotlyjs='cdn')

            qe_variance_chart_html = ""
            if fig_qe_variance is not None:
                qe_variance_chart_html = fig_qe_variance.to_html(full_html=False, include_plotlyjs='cdn')

            ba_variance_chart_html = ""
            if fig_ba_variance is not None:
                ba_variance_chart_html = fig_ba_variance.to_html(full_html=False, include_plotlyjs='cdn')

            portfolio_alloc_chart_html = ""
            if fig_portfolio_alloc is not None:
                portfolio_alloc_chart_html = fig_portfolio_alloc.to_html(full_html=False, include_plotlyjs='cdn')

            sub_portfolio_alloc_chart_html = ""
            if fig_sub_portfolio_alloc is not None:
                sub_portfolio_alloc_chart_html = fig_sub_portfolio_alloc.to_html(full_html=False, include_plotlyjs='cdn')

            brs_alloc_chart_html = ""
            if fig_brs_alloc is not None:
                brs_alloc_chart_html = fig_brs_alloc.to_html(full_html=False, include_plotlyjs='cdn')


            # For detailed project financials in report
            project_details_for_report = None
            fig_project_monthly_for_report = None
            if selected_project_name != 'Select a Project':
                project_details_for_report = filtered_df[filtered_df['PROJECT_NAME'] == selected_project_name].iloc[0]
                # Re-create the monthly breakdown for the report to ensure it's accurate
                monthly_breakdown_df_for_report = pd.DataFrame({
                    'Month': [f"2025_{i:02d}" for i in range(1, 13)],
                    'Actuals': [project_details_for_report.get(f'2025_{i:02d}_A', 0) for i in range(1, 13)],
                    'Forecasts': [project_details_for_report.get(f'2025_{i:02d}_F', 0) for i in range(1, 13)],
                    'Capital Plan': [project_details_for_report.get(f'2025_{i:02d}_CP', 0) for i in range(1, 13)]
                })
                monthly_project_melted_for_report = monthly_breakdown_df_for_report.melt(id_vars=['Month'], var_name='Type', value_name='Amount')
                fig_project_monthly_for_report = px.bar(
                    monthly_project_melted_for_report,
                    x='Month',
                    y='Amount',
                    color='Type',
                    barmode='group',
            ...
            report_content = generate_html_report(
                ...
                monthly_combined_df, fig_monthly_trends, fig_qe_variance, fig_ba_variance,
                fig_portfolio_alloc, fig_sub_portfolio_alloc, fig_brs_alloc,
                selected_project_name, project_details_for_report, fig_project_monthly_for_report
            )
